...
def setup_buttons(main_frame, project_dropdown, root, projects, reports_path, users, user_dropdown, create_project_folder, generate_report, get_current_user, base_path):
    ...
    add_project_button = tk.Button(button_frame, text="Add Project", command=lambda: add_project(root, projects, project_dropdown, create_project_folder, reports_path, base_path))
    add_project_button.pack(side="left", padx=5)

    add_user_button = tk.Button(button_frame, text="Add User", command=lambda: add_user(root, users, user_dropdown, base_path))
